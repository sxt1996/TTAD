function  Mass_list = get_shared_node_Mass_list( x,target,isolation_forest)
% get masses of the shared nodes of the testing pixel and the given target on multiple trees
% Input:
% x: testing pixel; target: a priori target spectrum; isolation_forest:multiple constructed trees 
% Output:
% Mass_list: masses of the shared nodes of the testing pixel and the given tartget on multiple trees


tree_use = isolation_forest{1};

band_num = length(x);   
tree_num  = length(isolation_forest);  

Mass_list = zeros(1,tree_num);

for i =1:tree_num    
    tree = isolation_forest{i};
